...
register:
	kubectl -n openhorizon-agent exec -i `kubectl -n openhorizon-agent get pods --selector=app=agent -o jsonpath={.items[*].metadata.name}` -- hzn register -u $HZN_EXCHANGE_NODE_AUTH --pattern "$(HZN_ORG_ID)/pattern-$(SERVICE_NAME)-$(ARCH)"
	@echo " "
	@echo " "
